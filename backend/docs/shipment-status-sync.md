...
class OrderStatus(StrEnum):
    ...
    CONFIRMED = "confirmed"                        # Подтверждён, готов к отправке
...
```
PENDING_CONFIRMATION → CONFIRMED → SHIPPED → READY_FOR_PICKUP → DELIVERED
                                      │
                                      └→ CLIENT_DONT_PICKUP → RETURNED_TO_SUPPLIER
```
...
- `PENDING_CONFIRMATION → CONFIRMED` — клиент подтверждает через сайт
