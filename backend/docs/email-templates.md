...
│   ├── PENDING_CONFIRMATION/         ← Заказ создан, ждёт подтверждения
...
1. Происходит событие (например, `order_status_changed` с `new_status = "confirmed"`)
2. Обработчик определяет `order_type` заказа (`CODFLOW` или `PREPAID`)
3. Ищет шаблоны: `templates/email/CODFLOW/CONFIRMED/*.template`
...
| CONFIRMED | client_confirm | Клиент | Заказ подтверждён |
| CONFIRMED | admin_confirm | Админ | Клиент подтвердил заказ (нал. платёж) |
