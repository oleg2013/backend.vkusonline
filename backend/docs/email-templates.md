...
Каждое составное поле имеет две версии: plain text и HTML (с суффиксом `_HTML`).
...
| `#ORDER_LIST#`                   | Состав заказа с ценами (plain text)        | Многострочный текст с позициями, доставкой, итого  |
| `#ORDER_LIST_HTML#`              | Состав заказа с ценами (HTML таблица)      | Styled HTML table                                  |
| `#ORDER_LIST_WITHOUT_PRICE#`     | Состав заказа без цен (plain text)         | Только названия товаров и количество               |
| `#ORDER_LIST_WITHOUT_PRICE_HTML#`| Состав заказа без цен (HTML таблица)       | HTML table без колонки цен и итогов                |
| `#PRICE#`                        | Итоговая сумма                             | `650.00 руб.`                                      |
| `#UNIQUE_ORDER_ID#`              | Токен для публичной ссылки на заказ        | `a1b2c3d4...`                                      |

`#ORDER_LIST_WITHOUT_PRICE#` / `#ORDER_LIST_WITHOUT_PRICE_HTML#` используются в шаблонах PREPAID для уведомлений о движении заказа (SHIPPED, READY_FOR_PICKUP) — клиент уже оплатил, видеть цены повторно не нужно.
...
| `#PVZDETAILS#`     | Детали ПВЗ (plain text: адрес, режим, телефон)          | Многострочный текст        |
| `#PVZDETAILS_HTML#`| Детали ПВЗ (HTML-карточка: адрес, режим, телефон)       | Styled HTML block          |

`#PVZDETAILS#` и `#PVZDETAILS_HTML#` автоматически подгружают данные из кеша ПВЗ (`pickup_points_cache`) по `delivery_provider` + `pickup_point_id` заказа. В HTML-шаблонах используйте `#PVZDETAILS_HTML#`, для plain text — `#PVZDETAILS#`. Содержимое зависит от провайдера:
...
### Поля условий доставки

Описание условий получения заказа для каждой службы доставки. Значения задаются в `.env`.

| Плейсхолдер                          | Описание                                        | Пример значения                       |
|--------------------------------------|-------------------------------------------------|---------------------------------------|
| `#DELIVERY_SERVICE_DESCRIPTION#`     | Условия получения (plain text)                  | Многострочный текст с инструкциями    |
| `#DELIVERY_SERVICE_DESCRIPTION_HTML#`| Условия получения (HTML-блок с синей рамкой)    | Styled HTML block                     |

Значения определяются переменными `.env`:

| Переменная               | Провайдер | Описание                                        |
|--------------------------|-----------|--------------------------------------------------|
| `MAGNIT_DELIVERY_TERMS`  | Магнит    | Условия получения в Магнит Пост                  |
| `FIVEPOST_DELIVERY_TERMS`| 5Post     | Условия получения в 5Post                        |

По умолчанию (если не задано в `.env`):
- **Магнит**: СМС с кодом и номером заказа, без подтверждения личности, хранение 5 дней
- **5Post**: СМС с кодом, без подтверждения личности, получение по коду, хранение 7 дней

Используется в шаблонах SHIPPED и READY_FOR_PICKUP (после `#PVZDETAILS_HTML#`).

**API endpoint:** `GET /api/v1/checkout/delivery-terms/{provider}` — возвращает текст условий для фронтенда.
...
| `#SHOP_PHONE#`     | Контактный телефон магазина             | `SHOP_PHONE` в .env      |
| `#SYS_SHOP_EMAIL#` | Email отправителя (SMTP)               | `SMTP_FROM_EMAIL` в .env |

`#SHOP_PHONE#` отображается в footer всех писем (layout `_layout.html`) и в шаблоне сброса пароля.
